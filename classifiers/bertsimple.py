...
def run_bert_for_extract_meta(meta, classes, bert, tokenizer, batch_size=128, device='cpu'):
    features = []  # document embeddings (= a list of [CLS] embeddings)
    labels = []    # class labels

    wordlists = meta['wordlists']
    wordlists = [s for s in wordlists if len(s) > 0]  # remove empty sentences
    if len(wordlists) == 0: 
        return None
    # read class labels if available
    if 'classes' in meta:
        labels.append(torch.tensor([min(1, meta['classes'][c]) for c in classes], dtype=torch.float))  # truncate values > 1
...
def test_mode(args, classes):
    ...
    classifier.load_state_dict(torch.load(args.model_path, map_location=args.device))
    classifier.eval()
    classifier.to(args.device)
...
def extract_meta_add_bert_info(meta, classes, bert, classifier, tokenizer, batch, device):
    features, _ = run_bert_for_extract_meta(meta, classes, bert, tokenizer, batch, device)
    num_test = len(features)
    ...
    outputs = []  # list of class probabilities
    with torch.no_grad():
        for i in range(0, len(features), batch):  # run classifier on minibatch
            batch, mask = make_batch(features[i:i+batch])
    ...
    # output class probabilities
    meta["classes_kwd"] = meta["classes"]
    meta["classes_bert"] = {label: val.item() for label, val in zip(classes, outputs[0])}
    meta["classes"] = {}
    for k, v in meta["classes_bert"].items():
        newv = 1 if v>=0.5 else 0
        meta["classes"][k] = newv
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--gpu", type=int, default=-1, help="Use GPU")
